...
class BaseMetricsLogger(Logger):
    ...
    _plotter: Optional[MetricsPlotter] = None
    ...
    def initialize(self, training_program):
        ...
        if self.save_plots and not self.only_metrics:
            self._plotter = MetricsPlotter(
                logger_name=self.name,
                plot_save_dir=self._plot_save_dir,
                plot_dpi=self.plot_dpi,
                plot_training_losses=self.plot_training_losses
            )
    ...
    def _log_metrics_step(self, step: int, step_data: Dict[str, Any], training_loss: Optional[Any] = None):
        ...
        try:
            ...
            # Plot if needed
            if self._should_plot_now(step) and self._plotter is not None:
                self._plotter.plot_metrics_history(self._metrics_history, step)
    ...
    def finalize(self):
        ...
        if self.plot_at_the_end and self._plotter is not None and self._metrics_history is not None:
            logger.info(f"{self.__class__.__name__} {self.name}: Creating final plot...")
            self._plotter.create_final_plot(self._metrics_history)
