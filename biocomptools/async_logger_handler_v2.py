...
class AsyncLoggerHandlerV2(BaseModel):
    """Multi-process async logger handler with ordering guarantees."""
    ...
    _aggregation_lock: Any = Field(exclude=True, default=None)
    _next_step_to_aggregate: int = Field(exclude=True, default=1)
    _pending_results: List = Field(exclude=True, default_factory=list)  # heapq
    _loggers_by_name: Dict[str, Any] = Field(exclude=True, default_factory=dict)
    ...
    def model_post_init(self, __context):
        ...
        logger.info(f"AsyncLoggerHandlerV2: {mode_info}, {batch_info}, tmpdir: {self.tmpdir}")
    ...
    def _setup_infrastructure(self):
        ...
        self._next_step_to_aggregate = 1
        self._pending_results = []

        # Build logger lookup
        self._loggers_by_name = {}
        for _, _, logger_obj in self.logger_callbacks:
            name = type(logger_obj).__name__
            self._loggers_by_name[name] = logger_obj
    ...
    def _classify_callbacks(self):
        ...
        for _, callback, logger_obj in self.logger_callbacks:
            name = type(logger_obj).__name__
    ...
    def _try_aggregate_ordered(
        ...
        with self._aggregation_lock:
            while True:
                next_step = self._next_step_to_aggregate
                ...
                for result in results:
                    ...
                    logger_obj = self._loggers_by_name.get(result.logger_name)
                ...
                self._next_step_to_aggregate = next_step + 1
    ...
    def shutdown(self, timeout: float = 30.0):
        ...
        logger.info("Shutting down AsyncLoggerHandlerV2...")
        ...
        self.cleanup()
        logger.info("AsyncLoggerHandlerV2 shutdown complete")
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'next_step_to_aggregate': self._next_step_to_aggregate,
...
class AggregatingLoggerMixin:
    ...
    _aggregation_lock: Lock = None

    def __init_subclass__(cls, **kwargs):
        super().__init_subclass__(**kwargs)
        cls._aggregation_lock = Lock()

    def _aggregate(self, step: int, metrics: Dict[str, Any]):
        """Override this to aggregate results from parallel callbacks.

        Thread-safe via _aggregation_lock.
        """
